...
class Sequence_RGBT:
    ...
    def _ensure_start_frame(self):
        ...
        if start_frame > 0:
            ...
            if self.ground_truth_seg is not None:
                self.ground_truth_seg = self.ground_truth_seg[start_frame:]
                assert len(self.frames_v) == len(self.frames_i) == len(self.ground_truth_seg)
    ...
    def __repr__(self):
        return "{self.__class__.__name__} {self.name}, length={len} frames".format(self=self, len=len(self.frames_v))
