...
from lib.models.layers.attn import MaskedCrossAttention
...
class CEABlock_single(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        # x[:,:lens_t,:] = fused_template 
        # xi[:,:lens_t,:] = fused_template 
        ...
        return x, global_index_template, global_index_search, removed_index_search, attn#, xi, global_index_templatei, global_index_searchi, removed_index_searchi, i_attn, KL
...
class CEABlock_SelfEnhancement(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.adap_MCA1 = MaskedCrossAttention(dim)
        self.adap_MCA2 = MaskedCrossAttention(dim)
    ...
    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        prompt_x, prompt_xi, KL1 = self.adap_MCA1(self.norm1(x), self.norm1(xi), ce_template_mask, lens_t)
        
        x_attn, attn = self.attn(self.norm1(x), mask, True)   
        x = x + self.drop_path(x_attn) + prompt_x

        xi_attn, i_attn = self.attn(self.norm1(xi), mask,True)
        xi = xi + self.drop_path(xi_attn) + prompt_xi
        ...
        prompt_x, prompt_xi, KL2 = self.adap_MCA2(self.norm2(x), self.norm2(xi), ce_template_mask, lens_t)

        x = x + self.drop_path(self.mlp(self.norm2(x))) + prompt_x

        xi = xi + self.drop_path(self.mlp(self.norm2(xi))) + prompt_xi
