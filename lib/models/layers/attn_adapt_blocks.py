...
from lib.models.layers.attn import MaskedSpatialCrossAttention
...
class CEABlock(nn.Module):
    ...
    def forward(self, n,x, xi, in_dict, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        
        xori = x
        
        lens_t = global_index_template.shape[1]
        fused_template = (x[:,:lens_t,:] + xi[:,:lens_t,:]) * 0.5
        x = torch.cat([fused_template, x[:,lens_t:,:]], dim=1)
        xi = torch.cat([fused_template, xi[:,lens_t:,:]], dim=1)
...
class CEABlock_SelfEnhancement(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # self.adap_MCA1 = MaskedCrossAttention(dim, 768)
        # self.adap_MCA2 = MaskedCrossAttention(dim, 768)
        self.adap_MCA1 = MaskedSpatialCrossAttention(dim)
        self.adap_MCA2 = MaskedSpatialCrossAttention(dim)
        # self.adap_norm1 = norm_layer(dim)
        # self.adap_norm2 = norm_layer(dim)
    ...
    def forward(self,n, x, xi, in_dict, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        # prompt = self.adap_MCA1(self.norm1(x), self.norm1(xi), in_dict, ce_template_mask, lens_t)
        # if n in [6,9]: prompt = 0
        x_attn, attn = self.attn(self.norm1(x), mask, True)   
        x = x + self.drop_path(x_attn) #+ prompt

        xi_attn, i_attn = self.attn(self.norm1(xi), mask,True)
        xi = xi + self.drop_path(xi_attn) #+ prompt
        ...
        # prompt = self.adap_MCA2(self.norm2(x), self.norm2(xi), in_dict, ce_template_mask, lens_t)
        # if n in [6,9]: prompt = 0
        x = x + self.drop_path(self.mlp(self.norm2(x))) #+ prompt

        xi = xi + self.drop_path(self.mlp(self.norm2(xi))) #+ prompt
