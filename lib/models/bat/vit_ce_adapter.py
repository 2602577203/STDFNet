...
from ..layers.attn_adapt_blocks import CEABlock_SelfEnhancement

from lib.models.layers.adapter import Bi_direct_adapter

_logger = logging.getLogger(__name__)

from lib.models.layers.patch_embed import SE_PatchEmbed

from lib.models.layers.attn import Fusion
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.adap_patch_embed = SE_PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans*2, embed_dim=embed_dim)
        self.adap_Fusion = Fusion(embed_dim)

       

    def forward_features(self, z, x, in_dict, mask_z=None, mask_x=None,
        ...
        import numpy as np
        # 转换为PIL图像
        pil_image = ToPILImage()(z_rgb[0])
        pil_image.save('z_rgb.jpg')
        pil_image = ToPILImage()(z_dte[0])
        pil_image.save('z_dte.jpg')
        ...
        fused_template = self.adap_patch_embed(torch.cat([z, zi], dim=1))
        if in_dict["dynamic_template"] is not None:
            dynamic_template_rgb = in_dict["dynamic_template"][:, :3, :, :]
            dynamic_template_tir = in_dict["dynamic_template"][:, 3:, :, :]
            z = torch.cat([z, dynamic_template_rgb], dim=0)
            zi = torch.cat([zi, dynamic_template_tir], dim=0)
        ...
        #     import numpy as np
        #     # 转换为PIL图像
        #     pil_image = ToPILImage()(dynamic_template_rgb[0])
        #     pil_image.save('dynamic_template_rgb.jpg')
        #     pil_image = ToPILImage()(dynamic_template_tir[0])
        #     pil_image.save('dynamic_template_tir.jpg')
            
        #     dynamic_template_rgb = self.patch_embed(dynamic_template_rgb)
        #     dynamic_template_tir = self.patch_embed(dynamic_template_tir)
        #     z = dynamic_template_rgb
        #     zi = dynamic_template_tir
        #     ce_template_mask = in_dict["dynamic_template_mask"]
        #     in_dict["dynamic_template"] = None
...
###################################################################===========
        ...
        #################################--------------
        fused_template += self.pos_embed_z
        ...
        if in_dict["dynamic_template"] is not None:
            z = z.reshape(1,128,768)
            zi = zi.reshape(1,128,768)
        ...
        x = torch.cat([fused_template, x], dim=1)
        xi = torch.cat([fused_template, xi], dim=1)
        
        # if in_dict["dynamic_template"] is not None:
        #     x = torch.cat([x, dynamic_template_rgb], dim=1)
        #     xi = torch.cat([xi, dynamic_template_tir], dim=1)
            # x = torch.cat([x, in_dict["tokens_target_rgb"].unsqueeze(0)], dim=1)
            # xi = torch.cat([xi, in_dict["tokens_target_tir"].unsqueeze(0)], dim=1)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            xi, global_index_ti, global_index_si, removed_index_si, attn_i = \
                blk(i,x, xi, in_dict, global_index_t, global_index_ti, global_index_s, global_index_si, mask_x, ce_template_mask,
                    ce_keep_rate)
            # KL_loss += KL
        ...
        # if in_dict["dynamic_template"] is not None:
        #     x = x[:,:fused_template.size(1)+lens_z+lens_x,:]
        #     xi = xi[:,:fused_template.size(1)+lens_z+lens_x,:]
        
        x = self.norm(x)
        xi = self.norm(xi)
        
        x = x[:,fused_template.size(1):,:]
        xi = xi[:,fused_template.size(1):,:]
        
        fused_template = x[:,:fused_template.size(1),:]
        ...
        if in_dict["dynamic_template"] is not None:
            lens_z_new = lens_z_new*2
            lens_zi_new = lens_zi_new*2
        ...
        # x = x + xi
        x = self.adap_Fusion(x, xi, 64, fused_template)
        ...
        aux_dict = {
            ...
            "search_tokens_rgb": search_tokens_rgb,
            "search_tokens_tir": search_tokens_tir,
            # "KL": KL_loss,
        }

        return x, aux_dict

    def forward(self, z, x, in_dict, ce_template_mask=None, ce_keep_rate=None,
                tnc_keep_rate=None,
                return_last_attn=False):

        x, aux_dict = self.forward_features(z, x, in_dict, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,)
