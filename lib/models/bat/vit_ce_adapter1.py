...
from ..layers.attn_adapt_blocks1 import CEABlock   ##BAT 
from ..layers.dualstream_attn_blocks import DSBlock ## Dual Stream without adapter

from ..layers.attn_adapt_blocks1 import CEABlock_SelfEnhancement
...
from lib.models.layers.patch_embed import SE_fusion

from ..layers.attn_adapt_blocks1 import CEABlock_single
...
###################################################################===========
        ...
        #     xi, global_index_ti, global_index_si, removed_index_si, attn_i, KL = \
        ...
        x = self.adap_SE_fusion(x, xi)
        
        for i, blk in enumerate(self.blocks):
            if i>=6:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask,
                        ce_keep_rate)
                #print(i, ": ", blk.keep_ratio_search)
                if self.ce_loc is not None and i in self.ce_loc:
                    removed_indexes_s.append(removed_index_s)
