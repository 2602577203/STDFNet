...
from lib.train.dataset import VisEvent, LasHeR, DepthTrack,LasHeR_testingSet,LasHeR_trainingSet
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        if name == "LasHeR_val":
            datasets.append(LasHeR_testingSet(settings.env.lasher_dir))
        if name == "VisEvent":
...
def get_optimizer_scheduler(net, cfg):
    ...
    if 'bat' in train_type:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "adap"  in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "adap" not in n and p.requires_grad],
